...
echo "[+] Scénario 1 : Simulation de Bruteforce SSH (Échecs de connexion)"
# Note : Pour que /var/log/auth.log reçoive l'alerte, on simule l'enregistrement de sshd
logger -p authpriv.notice "sshd[1234]: Failed password for fakeuser from 192.168.56.20 port 54321 ssh2"
logger -p authpriv.notice "sshd[1234]: Failed password for fakeuser from 192.168.56.20 port 54321 ssh2"
logger -p authpriv.notice "sshd[1234]: Failed password for fakeuser from 192.168.56.20 port 54321 ssh2"
logger -p authpriv.notice "sshd[1234]: Failed password for fakeuser from 192.168.56.20 port 54321 ssh2"
echo "  -> Logs d'échecs SSH injectés."

echo "[+] Scénario 2 : Usurpation via Sudo"
